...
namespace pico_sht3x {
    ...
    void SHT3X::init_2hz() {
        ...
        this->cmd_buffer[0] = 0x22;
        this->cmd_buffer[1] = 0x36;

        i2c_write_blocking( this->i2CInst, this->address, this->cmd_buffer, 2, false );
    }

    void SHT3X::read_raw( uint32_t &temp, uint32_t &humid )
    {
        this->cmd_buffer[0] = 0xe0;
        this->cmd_buffer[1] = 0x00;

        uint8_t buf[6];

        i2c_write_blocking( this->i2CInst, this->address, this->cmd_buffer, 2, true );
